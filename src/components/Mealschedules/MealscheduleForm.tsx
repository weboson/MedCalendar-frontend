...
// метод переключения на пункт в submenu
  ...
  const createHandler: SubmitHandler<IMealSchedule> = async (
    ...
    try {
      ...
      if (response) {
        // localStorage.setItem('idMealschedules', JSON.stringify(response.id)) //! раньше хранил в localStorage, теперь достаю из БД (getAll()), ведь в массиве "Mealschedules" в БД - есть только один график, т.к. (@OneByOne: MedCalendar-backend\src\mealschedule\entities\mealschedule.entity.ts)
